...
class AppriseSummary(Enum):
    ...
    never send summary 
    """
def wait_for_javascript(driver, delay=5000):
    """Wait for JavaScript challenges (e.g., Cloudflare) to complete."""
    logging.info(f"Waiting for {delay/1000} seconds for JavaScript challenges to finish...")
    driver.execute_script(f"return new Promise(resolve => setTimeout(resolve, {delay}));")
...
def executeBot(currentAccount: Account, args: argparse.Namespace):
    ...
    if args.searchtype in ("desktop", None):
        with Browser(mobile=False, account=currentAccount, args=args) as desktopBrowser:
            ...
            wait_for_javascript(desktopBrowser.webdriver, delay=10000)  # 10 Sekunden warten
            
            startingPoints = utils.getAccountPoints()
            logging.info(f"[POINTS] You have {utils.formatNumber(startingPoints)} points on your account")
            #time.sleep(30000)
    ...
    logging.info(f"[POINTS] You have earned {Utils.formatNumber(accountPoints - startingPoints)} points this run !")
    logging.info(f"[POINTS] You are now at {Utils.formatNumber(accountPoints)} points !")
